...
NF_NAMESPACE="default" # Namespace where the NF is deployed
...
# Wait for the condition/status to hit Running
# Using kubectl wait to block until status is reached
kubectl wait --namespace="${NF_NAMESPACE}" \
...
echo "--------------------------------------------------"
echo "Network Function '${RESOURCE_NAME}' has been deployed successfully!"
